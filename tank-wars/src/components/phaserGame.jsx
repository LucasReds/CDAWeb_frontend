...
class Example extends Phaser.Scene {
    ...
    create() {
        ...
        this.terrainCollider = this.matter.add.fromVertices(0, 0, this.verts, { isStatic: true }, true, 0.01, 10);
        this.matter.alignBody(this.terrainCollider, this.sys.game.config.width / 2, this.sys.game.config.height, Phaser.Display.Align.BOTTOM_CENTER);
        ...
        this.matter.world.on('collisionstart', (event) => {
            event.pairs.forEach(pair => {
                ...
                if ((gameObjectA instanceof Bullet && gameObjectB === this.player1) || (gameObjectB instanceof Bullet && gameObjectA === this.player1)) {

                    this.handleTankHit(this.player1, 'player1', this.healthPlayer1);
                };

                if ((gameObjectA instanceof Bullet && gameObjectB === this.player2) || (gameObjectB instanceof Bullet && gameObjectA === this.player2)) {
                    this.handleTankHit(this.player2, 'player2', this.healthPlayer2);
                };

                if ((gameObjectA instanceof Bullet || gameObjectB instanceof Bullet) && (bodyA.parent.id === this.terrainCollider.id || bodyB.parent.id === this.terrainCollider.id)) {
                    const bullet = gameObjectA instanceof Bullet ? gameObjectA : gameObjectB;
    ...
    updateHealthBar(healthBar, player, health) {
        ...
        healthBar.fillRect(x + remainingHealthWidth, y, lostHealthWidth, healthBarHeight);
    }
    ...
    isValidMove(x, y) {
        for (let i = 0; i < this.verts.length - 1; i++) {
            ...
            if ((x >= vert1.x && x <= vert2.x) || (x <= vert1.x && x >= vert2.x)) {
                ...
                if (Math.abs(y - yOnLine) <= 5) {  // Allow a small margin of error
                    return true;
                }
            }
        }
    ...
    explosionEffect(x, y) {
        ...
        this.time.delayedCall(this.explosionDuration, () => {
...
class Bullet extends Phaser.Physics.Matter.Image {
    constructor(scene, x, y) {
        ...
        this.scene.matter.world.on('collisionstart', (event, bodyA, bodyB) => {
            if ((bodyA === this.body && bodyB === this.scene.player1.body) || 
                (bodyB === this.body && bodyA === this.scene.player1.body)) {
                if (this.scene.isPlayer1Turn) {
                    return;
                }
            }
            if ((bodyA === this.body && bodyB === this.scene.player2.body) || 
